...
const ChatWithAssistant = () => {
  const [assiWaitingTime, setAssiWaitingTime] = useState({ min: 0, sec: 0 });
  const { setNodes } = useReactFlow();
  const { isActiveBottomTRForm } = useWorkFlowContextData();
  const handleChange = (e) => {
    const { name, value } = e.target;
    setAssiWaitingTime((prevData) => ({ ...prevData, [name]: value }));
  };
    ...
    setNodes((nds) =>
      nds.map((node) =>
        node.id === isActiveBottomTRForm.id
          ? {
              ...node,
              data: { ...node.data, message: assiWaitingTime },
            }
          : node
      )
    );
    console.log("assiWaitingTime", assiWaitingTime);
  };

  useEffect(() => {
    setAssiWaitingTime(isActiveBottomTRForm?.activeNode?.data?.message);
